...
function CarruselDiscografia({ items }) {
  ...
  useEffect(() => {
    ...
    const autoScroll = () => {
      if (carouselRef.current && !isDragging) {
        // Velocidad del movimiento automático. Aumenta para ir más rápido.
        carouselRef.current.scrollLeft += 0.8; 
        
        // Reinicio para el bucle infinito
        if (
  ...
  return (
    <div
      ...
      className="smx-hide-scrollbar"
      style={{
        display: "flex", overflowX: "auto", gap: 24, paddingBottom: 40,
        margin: "0 -5vw", padding: "0 5vw 40px",
        cursor: isDragging ? "grabbing" : "grab",
        /* Quitamos el scroll-snap porque interfiere con la animación fluida en JS */
      }}
    >
      {/* Duplicamos los items para que el scroll parezca infinito */}
      {[...items, ...items, ...items].map((item, i) => (
        <div 
          key={i} 
          style={{ 
            flex: "none", width: "clamp(260px, 30vw, 320px)",
            // Evitamos que un click accidental abra enlaces o imágenes mientras se arrastra
            pointerEvents: isDragging ? "none" : "auto" 
          }}
        >
          <DiscCard {...item} delay={0} />
        </div>
      ))}
    </div>
  );
